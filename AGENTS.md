# Agent Instructions (text-mining-nlp)

本仓库用于生成《文本知识挖掘与商务应用》教材的"每周章包"交付物（见 `chapters/`）。
...
核心要求：
- 场景驱动叙事，禁止模板化结构
- 每章必须有贯穿案例（渐进式文本分析任务）+ TextMine 超级线推进
...
| `example-engineer` | 示例代码 + 反例 + TextMine 超级线代码 | 与贯穿案例关联 |
...
| TextMine 演进连续 | example-engineer | 在上周基础上增量修改（系统/流水线不从头写） |
...
| `/qa-book` | 跨周一致性检查 |
| `/scaffold-book` | 批量创建 week_01..week_08 |

## Gitea PR 流程（Week 05+ 必做）
