...
| 阿码 | 第 4 节 | 追问"为什么不直接调用 API？封装一层不是多此一举吗？" | 解释封装的价值，引出抽象层的概念 |
...
更直观地说，我们直接看数据：100 条新闻，平均每条 300 字。用 BERT 微调，即使你熟悉流程，从数据预处理、模型训练到部署，至少需要几天时间（不算调参失败重来的）。而用 LLM API？10 分钟写好 Prompt，第一次调用就跑通。
...
| 开发周期 | 以周计（120+ 小时） | 以分钟计（10-60 分钟） |
| 部署复杂度 | 需要 GPU 服务器、版本管理 | 只需要 HTTP 请求 |
...
理解了"为什么变"，但问题来了——这要花多少钱？如果 API 调用贵得离谱，那"快速验证"也只是有钱人的游戏。答案可能会让你意外：2024-2025 年的价格战让这件事变得前所未有的便宜。我们先跳到一个"喘息点"，看看过去两年发生了什么。
...
看完价格革命，你应该对"用得起"有了信心。现在概念说得再多，不如亲手敲一次代码。我们直接用 OpenAI 的 API 来做一次调用。如果你用的是国产 LLM（智谱、通义千问、DeepSeek），代码结构几乎一样，后面会讲怎么切换。
...
现在你能让 LLM 说话了。但除了"聊天"，它还能帮你做什么？

你可能以为 LLM 只是对话机器人——问它问题，它回答你。其实它更像一个"文本处理器"：给它文本，告诉它你想要什么输出格式，它就能按你的要求加工。下一节我们看三种最常用的文本处理任务，它们可能完全改变你对"聊天机器人"的理解。

## 第 3 节：不只是聊天 —— 让 LLM 做分类、摘要和抽取

在舆情监控的场景里，你需要三种核心能力：分类、摘要、实体抽取。传统方法下，这需要训练三个不同的模型，每个模型都要收集数据、调参、部署。现在呢？同一个 LLM，换三段 Prompt 就行。

我们先详细看第一种任务，你会发现规律——然后其他两个就会触类旁通。
...
这段代码做了什么？`system` 消息告诉 LLM："你是分类助手，只能选这些类别，别废话"；`user` 消息塞入新闻文本；`temperature=0` 让输出稳定，不会这次说是"财经"，下次说是"科技"。

### 任务二和三：摘要和抽取（规律一样）

你发现规律了吗？**代码结构完全一样，只有 `system` 消息不同。**

**一模型多任务的小惊喜**：三年前，做这三个任务（分类、摘要、抽取）需要训练三个不同的模型，每个都要收集数据、调参、部署。现在？同一个 LLM，换三段 Prompt 就行。你不是在"调用 API"，而是在"设计系统"。
...
print(f"摘要：{summary}")
# 输出：苹果发布 2025Q1 财报，营收 1243 亿美元同比增长 4%，
...
这就是 LLM 的"一模型多任务"能力——传统方法需要训练三个模型，现在只需要改 Prompt。小惊喜：如果你以后要做情感分析、关键词提取、文本生成，也是同样的模式。
...
你现在能做三种任务了。但小北刚才来找我，说了一个痛苦的经历——他复制粘贴了 10 次同样的 API 调用代码，每次改一点，结果第 11 次忘了改一个参数，调试了半小时。
...
小北刚才的故事你听到了吗？他复制粘贴了 10 次 API 调用代码，每次改一点 system message，结果第 11 次忘了改 model 参数，用 GPT-4o 跑了一个只需要 GPT-4o-mini 的任务——账单多了三倍。

这不是"马虎"，这是"代码在等你犯错"。封装的目的不只是"少写代码"。更重要的是：统一错误处理、统一日志记录、方便切换 LLM 后端。如果你今天用 OpenAI，明天想换成智谱，只需要改一个地方。

```text
阿码追问：为什么不直接调用 API？封装一层不是多此一举吗？

好问题。想象一下：你的项目里有 50 个地方调用了 LLM API。突然有一天，OpenAI 改了接口，或者你要换成国产 LLM。如果没有封装，你得改 50 个地方；有了封装，只改 1 个地方。这就是"抽象层"的价值——它把"怎么做"和"做什么"分开了。
```
...
你的 Client 已经能稳定运行了。小北现在很高兴——他再也不会忘记改参数了。但老板刚才发了一封邮件："下周我们要处理 10000 条新闻，估算一下预算。"

小北的反应你可能猜到了："等等，我怎么知道这要花多少钱？"

下一节我们就来算账——别等月底账单吓一跳。

## 第 5 节：收到账单别慌 —— Token 计费与成本优化

假设老板让你处理 10000 条新闻，每条平均 500 字。在按下"运行"之前，你最好先算一下账——否则月底账单可能会让你心跳加速。

### Token 是什么

LLM 的计费单位不是"字"，而是 **Token**（词元）。

这听起来很抽象，我们用个类比：Token 就像"一口能说完的字数"。你读这段话时，大脑不会逐字逐句处理，而是会按"语意片段"来理解——"苹果公司/发布了/财报"是三个片段，而不是十个字。LLM 也是这样，它按 Token 处理文本。
...
以下是主流模型的价格对比（2026 年 2 月，实际价格可能变动）：
...
看到 DeepSeek 的价格没？输入只有 GPT-4o 的 5.6%，输出只有 2.8%。这个差距意味着什么？我们看个具体的例子。

### 成本估算实例：账单冲击

假设你要处理 1000 条新闻，每条 500 字，做分类、摘要、实体抽取三个任务。我们来算两笔账——用 GPT-4o 和用 DeepSeek。
...
# 摘要：400 输入，80 输出
# 抽取：450 输入，50 输出
# 三个任务总计：1250 输入，132 输出/条

# GPT-4o
gpt4o_cost = estimate_cost(1000, 1250, 132, 2.50, 10.00)
print(f"用 GPT-4o 处理 1000 条：${gpt4o_cost:.2f}")
# 输出：用 GPT-4o 处理 1000 条：$4.57

# DeepSeek-V3
deepseek_cost = estimate_cost(1000, 1250, 132, 0.14, 0.28)
print(f"用 DeepSeek 处理 1000 条：${deepseek_cost:.2f}")
# 输出：用 DeepSeek 处理 1000 条：$0.16
```

$4.57 vs $0.16——差距是一杯星巴克和一顿午餐的区别。

1000 条新闻，GPT-4o 要一杯拿铁的钱，DeepSeek 只要一瓶矿泉水。如果每天处理 1000 条，一个月下来：GPT-4o 是 $137，DeepSeek 是 $4.80。

老潘看到这个数字会说："在公司里，我们不会每次都调 GPT-4。简单任务用小模型就够了，成本差十倍。还有一个坑：很多人不知道 Prompt 本身也在烧钱。如果你写了 500 字的 system message，每次调用都要付这 500 字的钱。把 Prompt 精简一下，能省不少。"
