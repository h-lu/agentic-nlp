...
> "计算机能不能思考，这个问题并不比潜水艇能不能游泳更有趣。"
> — Edsger Dijkstra（图灵奖得主，算法先驱）
...
1. Agent 架构（Agent Architecture）— 感知、规划、执行、反思 — 首次出现：第 1 节
2. Function Calling（函数调用）— LLM 调用外部工具的能力 — 首次出现：第 2 节
3. ReAct 模式（Reasoning + Acting）— 推理与行动交替的循环 — 首次出现：第 3 节
4. 任务分解（Task Decomposition）— 将复杂任务拆解为可执行步骤 — 首次出现：第 4 节
5. 工具定义（Tool Definition）— 用 Schema 描述工具的能力 — 首次出现：第 2 节
...
注意：上面这些函数（`analyze_sentiment` 等）是**实际执行的函数**，它们定义了 Agent "能做什么"。简化起见，这里用词匹配方法，返回格式也比较简单（只有 `sentiment` 和 `score`）。生产环境的实现通常会包含更多字段（如 `confidence`、`reason`），你可以在第 4 节的 TextAgent Tools 中看到完整版本。
...
还记得阿码刚才说的 `exec()` 坑吗——LLM 可能写出 `os.system('rm -rf /')` 这种可怕代码？这就是 Function Calling 的核心价值：**安全可控**。你定义了哪些工具，LLM 就只能调这些工具；它想干别的，没门。
...
→ 返回：{"sentiment": "mixed", "confidence": 0.6, "reason": "正面词1个，负面词1个"}

系统把结果反馈给 LLM：
Observation: 情感分析结果是混合（mixed），置信度 0.6，原因：正面词1个，负面词1个
...
**重要：循环什么时候结束？** 当 LLM 认为已经收集到足够信息时，它会返回 `Final Answer:`（或代码实现中的 `Action: FINISH`），表示"我已经准备好给出最终答案了，不需要再调用工具"。这个"结束信号"是 ReAct 循环的关键——没有它，Agent 可能会无限循环下去。
...
**一句话总结**：ReAct = 思考 → 行动 → 观察 → 重复（直到能给出最终答案）。
...
4. 如果工具调用失败，在 Thought 中分析原因并尝试替代方案"""
    ...
    def _execute_tool(self, tool_name: str, tool_args: Dict) -> Dict:
        ...
        if tool_name == "analyze_sentiment":
            return {"sentiment": "positive", "confidence": 0.85, "reason": "正面词多于负面词"}
