...
## 范例 8：TextAgent 超级线进度写法

### ❌ 坏（与正文脱节）

> ## TextAgent 进度
>
> 本周给 TextAgent 加了 Prompt 模板。代码如下：
>
> ```python
...
> ## TextAgent 进度
>
> 到目前为止，TextAgent 有一个"看不见的坑"：我们把用户的原始问题直接丢给 LLM，但如果用户问的是"帮我分析这份数据"，LLM 可能会给出泛泛而谈的废话——因为它不知道我们期望什么样的输出格式。
>
> 这正好是本周"Prompt Engineering"派上用场的地方。与其裸奔式调用，不如先给 LLM 一个角色和输出约束：
>
> ```python
> SYSTEM_PROMPT = """你是一个数据分析助手。回答问题时：
> 1. 先确认数据范围和字段含义
> 2. 给出具体的数值或比例
> 3. 用一句话总结关键发现"""
>
...
> ```
>
> 现在 TextAgent 的回答会更结构化。系统提示约束了"怎么回答"，用户消息补充了"回答什么"，我们也能更有底气地把 LLM 输出写进分析报告。
>
> 老潘看到这段改动会说什么？"别指望 LLM 自己猜你的意图。把期望写清楚，它才能帮到你。"

**为什么好？**
- 从 TextAgent 的真实"痛点"出发，让本周知识有了用武之地。
- 改进有前后对比（之前裸奔调用，现在有系统提示和消息结构）。
- 用老潘的点评收束，增加工程视角。
